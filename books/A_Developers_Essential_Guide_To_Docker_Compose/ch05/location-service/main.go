...
func nearByLocations(c context.Context, longitude float64, latitude float64, unit string, distance float64) ([]LocationNearMe, error) {
	...
	var query = &redis.GeoRadiusQuery{Unit: unit, WithDist: true, Radius: distance, Sort: "ASC"}
...
func fetchLocation(c context.Context, id string) (*Location, error) {
	...
	location := Location{Id: ires["Id"], Name: ires["Name"], Description: ires["Description"], Latitude: latitude, Longitude: longitude}
...
func persistLocation(c context.Context, location Location) error {
	hmset := client.HSet(c,
		fmt.Sprintf(locationIdFormat, location.Id),
		location.Id,
		"Name", location.Name,
