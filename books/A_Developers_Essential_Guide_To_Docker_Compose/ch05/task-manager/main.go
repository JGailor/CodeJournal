...
func setupRouter() *gin.Engine {
	...
	r.GET("/task", func(c *gin.Context) {
		if tasks, err := taskService.FetchTasks(c.Request.Context()); err != nil {
	...
	r.GET("/task/:id", func(c *gin.Context) {
		...
		if task, err := taskService.FetchTask(c.Request.Context(), id); err != nil {
			...
			if task.Location != nil {
				if locationsNearMe, err := locationService.FindNearbyLocations(task.Location.Longitude, task.Location.Latitude, "km", 1.0); err != nil {
					c.JSON(http.StatusInternalServerError, gin.H{"id": id, "message": err.Error()})
				} else {
